...
def prepare_data(neural_data, run_onset, det_window, perc_test):
    ...
    perc_test: percentage of trials to use for testing data in crossvalidation
    ...
    for label in [run_onset, no_onset]:
        for event_idx, event in enumerate(label):
            if event ==1:         
                ...
                neural_act = neural_act.mean(axis =1).T # take the average activity of each window for all neurons
    ...
    # randomly shuffle into test and train; equalize number of movement and no movement
...
X_train, y_train, X_test, y_test = prepare_data(feat, run_onset, det_window, 0.1)
