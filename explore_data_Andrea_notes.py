...
movement_thr = dat["run"].mean() # 3.97
movement_thr = np.median(dat["run"]) # 0.018
#movement_thr = np.percentile(dat["run"], 90)
detected_movement = (filtered_run > movement_thr).astype(int) # apply the threshold; returns a Boolean
